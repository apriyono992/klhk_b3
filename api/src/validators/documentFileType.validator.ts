...
interface IsDocumentValidFileOptions {
...
export class IsDocumentValidFile implements ValidatorConstraintInterface {
  private readonly supportedFormats: string[];
  private readonly maxFileSize: number;

  constructor(options: IsDocumentValidFileOptions = {}) {
    // Set default values but allow for customization via options
    this.supportedFormats = options.supportedFormats || ['application/pdf',
    'application/msword',
  ...
  validateAndThrow(files: Express.Multer.File[]): void {
    ...
    if (!files || files.length === 0) {
      throw new BadRequestException('Documents cannot be empty or undefined. Please upload at least one valid file.');
    }
