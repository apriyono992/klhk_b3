...
export default function IndexPage() {
    ...
    const { data, isLoading, mutate } = useSWR(
        `/api/content/search-articles?page=${page + 1}&limit=${pageSize}`,
        getFetcher
    )

    const { data: categoryList, isLoading: categoryListLoading } = useSWR(
        '/api/content/category?type=ARTICLE',
        getSelectFetcher
    )
    ...
    const articleImages = watch('attachments', [])
    ...
    return (
        <RootAdmin>
            <Card className="w-full mt-3" radius="sm">
                <CardHeader>
                    <p className="text-md">Daftar Artikel</p>
                </CardHeader>
                <Divider />
            ...
            >
                <ModalContent className="max-h-[90vh] overflow-y-auto">
                    {(onClose) => (
                        <>
                            <ModalHeader>{isEdit ? 'Ubah' : 'Tambah'} Artikel</ModalHeader>
                            <ModalBody>
