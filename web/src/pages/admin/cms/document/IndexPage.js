...
export default function IndexPage() {
    ...
    const { data, isLoading, mutate } = useSWR(
        `/api/content/search-document?page=${page + 1}&limit=${pageSize}`,
        getFetcher
    )

    // Fetch category of document
    const { data: categoryList, isLoading: categoryListLoading } = useSWR(
        '/api/content/category?type=DOCUMENT',
        getSelectFetcher
    )
    ...
    return (
        <RootAdmin>
            <Card className="w-full mt-3" radius="sm">
                <CardHeader>
                    <p className="text-md">Daftar Dokumen</p>
                </CardHeader>
                <Divider />
            ...
            >
                <ModalContent className="max-h-[90vh] overflow-y-auto">
                    {(onClose) => (
                        <>
                            <ModalHeader>{isEdit ? 'Ubah' : 'Tambah'} Dokumen</ModalHeader>
                            <ModalBody>
