...
export default function IndexPage() {
  ...
  const { data: reportedCompanies, mutate: mutateReportedCompanies } = useSWR("/api/period/search/companies?isReported=true", getFetcher, swrOptions);

  const { data: unreportedCompanies, mutate: mutateUnreportedCompanies } = useSWR("/api/period/search/companies?isReported=false", getFetcher, swrOptions);

  // Fetch data aplikasi yang sudah dan belum melaporkan
  const { data: reportedApplications, mutate: mutateReportedApplications } = useSWR("/api/period/search/applications?isReported=true", getFetcher, swrOptions);

  const { data: unreportedApplications, mutate: mutateUnreportedApplications } = useSWR("/api/period/search/applications?isReported=false", getFetcher, swrOptions);

  // Fetch data registrasi yang sudah dan belum melaporkan
  const { data: reportedRegistrations, mutate: mutateReportedRegistrations } = useSWR("/api/period/search/registrations?isReported=true", getFetcher, swrOptions);

  const { data: unreportedRegistrations, mutate: mutateUnreportedRegistrations } = useSWR("/api/period/search/registrations?isReported=false", getFetcher, swrOptions);
    ...
    () => [
      ...
      },
      {
        ...
        renderCell: (params) => {
          ...
          return (
            <Chip color={status === "Sudah" ? "success" : "danger"} variant="flat" size="sm">
              {status}
            </Chip>
          );
        },
        ...
        <CardBody className="w-full h-[550px] p-5">
          <Tabs aria-label="Pelaporan Pencarian B3" defaultValue="reportedCompanies">
            <Tab title="Pelaporan Bedasarkan Jenis B3" value="reportedCompanies">
              <CustomDataGrid data={reportedCompanies?.data || []} rowCount={reportedCompanies?.total || 0} columns={columnsPerusahaan} pageSize={pageSize} setPageSize={setPageSize} page={page} setPage={setPage} />
            </Tab>
            <Tab title="Pelaporan Bedasarkan Perusahaan" value="unreportedCompanies">
              <CustomDataGrid data={unreportedCompanies?.data || []} rowCount={unreportedCompanies?.total || 0} columns={columnsPerusahaan} pageSize={pageSize} setPageSize={setPageSize} page={page} setPage={setPage} />
            </Tab>
            <Tab title="Pelaporan Bedasarkan Perusahaan" value="unreportedCompanies">
              <CustomDataGrid data={unreportedCompanies?.data || []} rowCount={unreportedCompanies?.total || 0} columns={columnsPerusahaan} pageSize={pageSize} setPageSize={setPageSize} page={page} setPage={setPage} />
            </Tab>
            <Tab title="Pelaporan Bedasarkan Digunakan Dan Dibeli" value="unreportedCompanies">
              <CustomDataGrid data={unreportedCompanies?.data || []} rowCount={unreportedCompanies?.total || 0} columns={columnsPerusahaan} pageSize={pageSize} setPageSize={setPageSize} page={page} setPage={setPage} />
            </Tab>
            <Tab title="Pelaporan Bedasarkan Lokal Dan Import" value="unreportedCompanies">
              <CustomDataGrid data={unreportedCompanies?.data || []} rowCount={unreportedCompanies?.total || 0} columns={columnsPerusahaan} pageSize={pageSize} setPageSize={setPageSize} page={page} setPage={setPage} />
            </Tab>
          </Tabs>
        </CardBody>
