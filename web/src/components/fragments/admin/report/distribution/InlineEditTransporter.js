...
export default function InlineEditTransporter({ selectedOption, companyId, control, setValue }) {
    ...
    const header = ['Nama Perusahaan', 'Alamat', 'Actions'];
    const content = (item) => (
        <TableRow key={item.id}>
            <TableCell>{item.namaTransPorter}</TableCell>
    ...
    const { data: dataTransporter, isLoading: isLoadingTransporter, mutate: mutateTransporter } = useSWR(companyId ? `/api/company/search-data-transporter?companyId=${companyId}` : null, getFetcher);

    function onClickEdit(item) {    
        setEditId(item.id);
        setIsEdit(true);
        resetInline({
            namaTransporter: item.namaTransPorter,
            alamat: item.alamat,
            email: item.email,
            ...
            dataPICIds: item.DataPic.map(item => ({value: item.id, label: item.nama})), 
        });           
    ...
    async function onSubmitForm(formData) {
        try {
            if (isEdit) {
                ...
                toast.success('Perusahaan transporter berhasil diubah!');
                
            } else {
                formData.companyId = companyId
                const response =  await postFetcher('/api/company/create-transporter', formData);
                mutateTransporter()
                const newItem = {value: response.id, label: response.namaPerusahaan,};
                setSelectedItems(prevItems => [...prevItems, newItem]);
                setDetailSelectedItem()
                setValue("dataTransporters", [...selectedItems, newItem].map(item => item.value));
                toast.success('Perusahaan transporter berhasil ditambah!');
            }
            onCloseForm();
        } catch (error) {
    ...
    const handleSelectChange = (options) => {
        ...
        const filteredData = dataTransporter?.data?.filter(item => opt.includes(item.id));
        setDetailSelectedItem(filteredData)
        setSelectedItems(options || []);
        setValue("dataTransporters", opt);
    };
        ...
        <div className='flex flex-col'>
            <div className='flex items-end gap-3'>
                <div className='flex-1'>
                    <Controller
                        name="dataTransporters"
                        control={control}
                        render={({ field, fieldState }) => (
                            <div className='flex flex-col'>
                                <label className='text-sm mb-2'>Perusahaan Transporter <span className='text-danger'>*</span></label>
                                <ReactSelect
                                    isMulti
                                    options={dataTransporter?.data?.map((item) => ({value: item.id, label: item.namaTransPorter}))}
                ...
                <Button onPress={onOpenModalForm} color='primary' startContent={<PlusIcon className='size-4'/>}>Tambah Perusahaan Transport</Button>
            </div>
            <div className="mt-4">
                        ...
                        <>
                            ...
                            <ModalBody>
                                    <div className='grid grid-cols-1 md:grid-cols-2 gap-3 mb-6'>  
                                        <ControlledInput label="Transporter" name="namaTransporter" type="text" isRequired={true} control={controlInline} />
                                        ...
                                        <div className="col-span-2">
                                            <DataPICSelector control={controlInline} setValue={setValueInline} type={'transporter'} companyIds={[companyId]} />
                                        </div>                   
                                    </div>
