...
export default function TableMaterial({ data, isLoading, mutate }) {
    ...
    const list = (item) => [
        ...
        {
            'label': 'Asal Muat',
            'value': item.asalMuatLocations.map((location, index) => (<div key={index}>{index+1}. {location.name}</div>))
        },
        {
            'label': 'Tujuan Bongkar',
            'value': item.tujuanBongkarLocations.map((location, index) => (<div key={index}>{index+1}. {location.name}</div>))
        },
        ...
        <>
            <Card radius="sm">
                <CardHeader className="flex items-center gap-3">
                    ...
                    <Button isIconOnly onPress={() => onClickCreate(data.id)} size="sm" color="primary"><PlusIcon className="size-4 stroke-2"/></Button>
                </CardHeader>
                <Divider />
                <CardBody>
                        ...
                        <TableBody loadingContent={<Spinner/>} loadingState={isLoading ? 'loading' : 'idle'} emptyContent="Tidak ada data">
                            {data?.b3Substances.map((item, index) => (
                                <TableRow key={index}>
                                    ...
                                    <TableCell>{item.dataBahanB3.casNumber} / {item.dataBahanB3.namaBahanKimia}</TableCell>
            ...
            <Modal scrollBehavior="inside" size="5xl" isOpen={isOpenModalForm} onOpenChange={onOpenChangeModalForm} onClose={onCloseForm} isDismissable={false} isKeyboardDismissDisabled={false}>
                <ModalContent>
                    {(onClose) => (
                        <>
                            ...
                            <ModalBody>
                                <form onSubmit={handleSubmit(onSubmitForm)}>
                                    ...
                                    <div className='flex items-center gap-1'>
                                        <Button isLoading={isSubmitting} isDisabled={isSubmitting || (isEdit && Object.keys(dirtyFields).length === 0)} type='submit' color='primary'>Simpan</Button>
